terraform {
  source = "git::https://github.com/JamieTaffurelliOrg/az-landingzone-storage-tf///?ref=0.0.34"
}
...
provider "azurerm" {
  ...
  subscription_id = "${include.azure.locals.conn_shrd_subscription_id}"
...
locals {
  tags                  = merge(include.azure.locals.default_tags, include.landing_zone.locals.default_tags, include.environment.locals.default_tags, { environment = "shared", stack = "connectivity" })
  org_prefix            = include.azure.locals.org_prefix
  lz_environment_hyphen = "conn-shrd"
  lz_environment_concat = "connshrd"
...
inputs = {
  ...
  storage_account_name = "st${local.org_prefix}connshrdtf${local.location_short}001"
